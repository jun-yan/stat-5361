...
my_chisq_test <- function(x, bins = seq(0.1, 0.9, by = 0.1), B = 1000,
    ...
    getstat <- function(x, thetahat, bins) {
        pp <- diff(c(0, bins, 1))
        qx <- quantile(x, bins)
        p <- texmex::pgpd(qx, sigma = thetahat[1], xi = thetahat[2])
        freq <- diff(n * c(0, p, 1))
    ...
    stat.b <- double(B)
      for (i in 1:B) {
        x.b <- texmex::rgpd(n, sigma = thetahat[1], xi = thetahat[2])
...
stat.ch <- replicate(1000, my_chisq_test(texmex::rgpd(n, sigma=theta[1], xi=theta[2]), B = 0,
                                         method = "minchisq"))
stat.ml <- replicate(1000, my_chisq_test(texmex::rgpd(n, sigma=theta[1], xi=theta[2]), B = 0,
                                         method = "ml"))
